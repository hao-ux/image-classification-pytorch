...
class GhostNetV2(nn.Module):
    def __init__(self, cfgs, num_classes=1000, width=1.0, dropout=0.2,block=GhostBottleneckV2,args=None, pretrained=True):
        ...
        if pretrained:
            checkpoint = torch.load('weights/ck_ghostnetv2_10.pth.tar')
...
@register_model
def ghostnetv2(num_classes=1000, pretrained=False):
    cfgs = [   
        # k, t, c, SE, s 
        [[3,  16,  16, 0, 1]],
        [[3,  48,  24, 0, 2]],
    ...
    return GhostNetV2(cfgs, num_classes=num_classes, 
                    width=1.0, 
                    dropout=0.2,
                    args=None, pretrained=pretrained)